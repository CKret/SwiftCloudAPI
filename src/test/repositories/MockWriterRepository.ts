...
export class MockWriterRepository implements IReadRepository<WriterEntity> {
    ...
    async findAll(): Promise<WriterEntity[]> { return this.data ?? null; }
    async findById(id: number): Promise<WriterEntity> { return this.data.find(x => x.ID === id) ?? null; }
    async findByName(name: string): Promise<WriterEntity> { return this.data.find(x => x.Name === name) ?? null; }
}
