    ...
    return {
        WriterRepository: jest.fn().mockImplementation(() => {
            return {
                ...
                async findAll(): Promise<PersonEntity[]> { return this.data ?? null },
                async findById(id: number): Promise<PersonEntity> { return this.data.find(x => x.ID === id) ?? null; },
                async findByName(name: string): Promise<PersonEntity> { return this.data.find(x => x.Name === name) ?? null; }
            }
        })
    };
});
...
describe('WriterService', () => {
    ...
    it('AutoMockedWriterRepository.findById returns 1 users', async () => {
        const service = new WriterService(new WriterRepository());
        expect(await service.findById(1, true)).toHaveProperty('ID', 1);
    });

    it('AutoMockedWriterRepository.findById returns 0 users', async () => {
        const service = new WriterService(new WriterRepository());
        expect(await service.findById(3, true)).toBeNull();
    });
    ...
    it('AutoMockedWriterRepository.findByName returns 0 users', async () => {
        const service = new WriterService(new WriterRepository());
        expect(await service.findByName('Taylor Swift')).toBeNull();
    });
});
